subroutine print_mos_input(mo_num_inp,mo_coef_inp)
 implicit none
 integer, intent(in) :: mo_num_inp
 double precision, intent(in) :: mo_coef_inp(ao_num,mo_num_inp)
...
! this is for the new version of molden
 ...
 do j = 1, mo_num_inp
  ...
  write(i_unit_output,*)''
  do i = 1, ao_num
!   write(i_unit_output,'(2X,I3, 2X A1,  I3, 2X A4 , F9.6)')i,trim(element_name(int(nucl_charge(ao_nucl(i))))),ao_nucl(i),(ao_l_char_space(i)),mo_coef_inp(i,j)
! F12.6 for larger coefficients...
   write(i_unit_output,'(2X,I3, 2X A2,  I3, 2X A4 , F12.6)')i,trim(element_name(int(nucl_charge(ao_nucl(i))))),ao_nucl(i),(ao_l_char_space(i)),mo_coef_inp(new_order_all_AOS_for_molden(i),j)
