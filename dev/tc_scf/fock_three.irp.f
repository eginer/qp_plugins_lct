...
!     int_1  += weight * w_kk * 1.5d0 * (mos_ll * w_ia + mos_ia * w_ll) 
     int_1  += weight * w_kk * 1.5d0 * (mos_ll * w_ia )
...
subroutine give_fock_ia_real_space_new(i,a,contrib)
 ...
 do mm = 1, 3
  do ipoint = 1, n_points_final_grid
   ...
   big_v_r  = 0.d0
   do k = 1, elec_beta_num
    double precision :: big_w_kk,rho_k,v_i,psi_k, psi_a,psi_i
    mos_k  = mos_in_r_array_transp(ipoint,k) 
    mos_kk = mos_k * mos_k 
    mos_ki = mos_k * mos_i
    mos_ka = mos_k * mos_a
...
!   int_1  += weight * big_w_kk * 1.5d0 * (rho_k * w_ia + mos_ia * big_w_kk)
   int_1  += weight * big_w_kk * 1.5d0 * (rho_k * w_ia )
!   int_2  += weight * big_w_kk * (1.5d0 * rho_k * w_ia - psi_i * mos_a - psi_a * mos_i)
