subroutine give_polynom_x_for_erf_int(                           &
  ...
  double precision, intent(in)   :: P_new(0:max_dim),P_center,fact_p,p,p_inv
  double precision, intent(in)   :: Q_new(0:max_dim),Q_center,fact_q,q,q_inv
...
double precision function general_primitive_integral_erf_new(dim,            &
  ...
  do m = 1, 3
   call give_polynom_x_for_erf_int(                                              &
       P_new(0,m),P_center(m),fact_p,p,p_inv,iorder_p(m),                        &
       Q_new(0,m),Q_center(m),fact_q,q,q_inv,iorder_q(m),                        &
       n_Ix(m), Ix_pol(0,m))
   if(n_Ix(m) == -1) then
    return
   endif
