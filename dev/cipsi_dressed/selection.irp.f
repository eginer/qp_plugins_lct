...
! -----
      ...
      if(cipsi_tc == "reg_h") then
        ...
        !
        ! Hermitian 
        ! c_alpha = <alpha|H|psi_0>/delta_E(alpha)
        ! e_alpha = c_alpha * <alpha|H|psi_0>
        !
        !   |psi_0>           |alpha> 
        ! | E0            |   alpha_h_psi |
        ! | alpha_h_psi   | Hii - E_shift |
        ! c_alpha, E0  + e_pert
...
!        enddo
        ...
        do iii = 1, N_det_thresh_psi_r
          ! coefficient left
          call hji_hij_mu_mat_tot(psi_det_sorted_r(1,1,iii), det, N_int, i_h_alpha,alpha_h_i)
          psi_h_alpha += i_h_alpha * leigvec_tc_sorted_r(iii,1)
          ! coefficient right
          alpha_h_psi += alpha_h_i * reigvec_tc_sorted_r(iii,1) 
        enddo
...
!       delta_E = E0(istate) - Hii + E_shift
        ...
        do iii = 1, N_det
          ...
          alpha_h_psi += alpha_h_i * reigvec_tc(iii,1) 
        enddo
        coef(istate)   = alpha_h_psi 
!        e_pert(istate) = 0.5d0 * (-delta_E - dsqrt(delta_E*delta_E +  4.d0 * psi_h_alpha * alpha_h_psi))
        e_pert(istate) = coef(istate) * psi_h_alpha
      else if(cipsi_tc == "e_sym") then
        ! -------------------------------------------
        ! Non hermitian 
        ! c_alpha = <alpha|H(j)|psi_0>/delta_E(alpha)
        ! e_alpha = c_alpha * <psi_0|H(j)|alpha>
        ...
        do iii = 1, N_det
          ! coefficient left
          call htilde_mu_mat_tot( psi_det(1,1,iii), det, N_int, i_h_alpha)
          psi_h_alpha += i_h_alpha * reigvec_tc(iii,1)
