subroutine mo_two_e_integrals_eff_pot_index(i,j,k,l,i1)
  use map_module
  implicit none
  BEGIN_DOC
...
BEGIN_PROVIDER [ logical, mo_two_e_integrals_eff_pot_in_map ]
  use map_module
  implicit none
  ...
  mo_two_e_integrals_eff_pot_in_map = .True.
!  if (read_mo_two_e_integrals_eff_pot) then
!    print*,'Reading the MO integrals_eff_pot'
!    call map_load_from_disk(trim(ezfio_filename)//'/work/mo_ints_eff_pot',mo_integrals_eff_pot_map)
...
!  endif
  ...
  !  call four_index_transform_block(ao_integrals_eff_pot_map,mo_integrals_eff_pot_map, &
  !      mo_coef, size(mo_coef,1),                                      &
  !      1, 1, 1, 1, ao_num, ao_num, ao_num, ao_num,                    &
  !      1, 1, 1, 1, mo_num, mo_num, mo_num, mo_num)
   call add_integrals_to_map_eff_pot(full_ijkl_bitmask_4)
    integer*8                      :: get_mo_eff_pot_map_size, mo_eff_pot_map_size
    mo_eff_pot_map_size = get_mo_eff_pot_map_size()

! print*,'Molecular integrals eff_pot provided:'
! print*,' Size of MO eff_pot map           ', map_mb(mo_integrals_eff_pot_map) ,'MB'
! print*,' Number of MO eff_pot integrals: ',  mo_eff_pot_map_size
!  if (write_mo_two_e_integrals_eff_pot) then
!    call ezfio_set_work_empty(.False.)
!    call map_save_to_disk(trim(ezfio_filename)//'/work/mo_ints_eff_pot',mo_integrals_eff_pot_map)
...
END_PROVIDER
 ...
 BEGIN_PROVIDER [ double precision, mo_two_e_int_eff_pot_jj_from_ao, (mo_num,mo_num) ]
&BEGIN_PROVIDER [ double precision, mo_two_e_int_eff_pot_jj_exchange_from_ao, (mo_num,mo_num) ]
&BEGIN_PROVIDER [ double precision, mo_two_e_int_eff_pot_jj_anti_from_ao, (mo_num,mo_num) ]
  BEGIN_DOC
  ...
  if (.not.do_direct_integrals) then
    PROVIDE ao_two_e_integrals_eff_pot_in_map mo_coef
  endif

  mo_two_e_int_eff_pot_jj_from_ao = 0.d0
  mo_two_e_int_eff_pot_jj_exchange_from_ao = 0.d0
  ...
  !$OMP PARALLEL DEFAULT(NONE)                                       &
      ...
      !$OMP REDUCTION(+:mo_two_e_int_eff_pot_jj_from_ao,mo_two_e_int_eff_pot_jj_exchange_from_ao)
  ...
  do s=1,ao_num
    do q=1,ao_num
      ...
      enddo
        ...
        do r=1,ao_num
          call get_ao_two_e_integrals_eff_pot_non_zero(q,r,s,ao_num,int_value,int_idx,n)
          do pp=1,n
            p = int_idx(pp)
          ...
          enddo
          call get_ao_two_e_integrals_eff_pot_non_zero(q,s,r,ao_num,int_value,int_idx,n)
          do pp=1,n
            p = int_idx(pp)
            integral = int_value(pp)
            if (abs(integral) > ao_integrals_threshold) then
      ...
      do i=1,mo_num
        ...
        do j=1,mo_num
          ...
          mo_two_e_int_eff_pot_jj_from_ao(j,i) += c * iqis(i)
          mo_two_e_int_eff_pot_jj_exchange_from_ao(j,i) += c * iqri(i)
        enddo
      enddo
  ...
  mo_two_e_int_eff_pot_jj_anti_from_ao = mo_two_e_int_eff_pot_jj_from_ao - mo_two_e_int_eff_pot_jj_exchange_from_ao
...
END_PROVIDER
 ...
 BEGIN_PROVIDER [ double precision, mo_two_e_int_eff_pot_jj, (mo_num,mo_num) ]
&BEGIN_PROVIDER [ double precision, mo_two_e_int_eff_pot_jj_exchange, (mo_num,mo_num) ]
&BEGIN_PROVIDER [ double precision, mo_two_e_int_eff_pot_jj_anti, (mo_num,mo_num) ]
  implicit none
  BEGIN_DOC
  ...
  mo_two_e_int_eff_pot_jj = 0.d0
  mo_two_e_int_eff_pot_jj_exchange = 0.d0

  do j=1,mo_num
    do i=1,mo_num
      mo_two_e_int_eff_pot_jj(i,j) = get_mo_two_e_integral_eff_pot(i,j,i,j,mo_integrals_eff_pot_map)
      mo_two_e_int_eff_pot_jj_exchange(i,j) = get_mo_two_e_integral_eff_pot(i,j,j,i,mo_integrals_eff_pot_map)
      mo_two_e_int_eff_pot_jj_anti(i,j) = mo_two_e_int_eff_pot_jj(i,j) - mo_two_e_int_eff_pot_jj_exchange(i,j)
    enddo
  enddo
...
subroutine clear_mo_eff_pot_map
  ...
  FREE mo_integrals_eff_pot_map mo_two_e_int_eff_pot_jj mo_two_e_int_eff_pot_jj_anti
  FREE mo_two_e_int_eff_pot_jj_exchange mo_two_e_integrals_eff_pot_in_map


end

subroutine provide_all_mo_integrals_eff_pot
  implicit none
  provide mo_integrals_eff_pot_map mo_two_e_int_eff_pot_jj mo_two_e_int_eff_pot_jj_anti
  provide mo_two_e_int_eff_pot_jj_exchange mo_two_e_integrals_eff_pot_in_map

end


subroutine add_integrals_to_map_eff_pot(mask_ijkl)
  use bitmasks
  implicit none
  ...
  !$OMP PARALLEL PRIVATE(l1,k1,j1,i1,i2,i3,i4,i,j,k,l,c, ii1,kmax,   &
      ...
      !$OMP  mo_coef,mo_integrals_threshold,mo_integrals_eff_pot_map)
  n_integrals = 0
  wall_0 = wall_1
  ...
  do l1 = 1,ao_num
    ...
    do k1 = 1,ao_num
      two_e_tmp_2 = 0.d0
      do j1 = 1,ao_num
        call get_ao_two_e_integrals_eff_pot(j1,k1,l1,ao_num,two_e_tmp_0(1,j1)) ! all integrals for a given l1, k1
        ...
        do k0 = 1, n_k
          ...
          do i0 = 1, n_i
            ...
            if (n_integrals == size_buffer) then
              call insert_into_mo_integrals_eff_pot_map(n_integrals,buffer_i,buffer_value,&
    ...
    if (thread_num == 0) then
      if (wall_2 - wall_0 > 1.d0) then
        ...
        print*, 100.*float(l1)/float(ao_num), '% in ',               &
            wall_2-wall_1, 's', map_mb(mo_integrals_eff_pot_map) ,'MB'
      endif
    endif
  enddo
  !$OMP END DO NOWAIT
  ...
  !$OMP END PARALLEL
  call map_merge(mo_integrals_eff_pot_map)

  call wall_time(wall_2)
  call cpu_time(cpu_2)
  integer*8                      :: get_mo_eff_pot_map_size, mo_map_size
  mo_map_size = get_mo_eff_pot_map_size()
