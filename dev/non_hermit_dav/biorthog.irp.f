...
subroutine non_hrmt_real_diag_random(n, A, leigvec, reigvec, n_real_eigv, eigval)
  ...
  !
  !
  ! WARNING : IN ORDER TO REMOVE EIGENVALUES WITH SMALL IMAGINARY PARTS, WE ADD RANDOM NUMBERS ON THE DIAGONAL OF A
  !
  END_DOC
...
end subroutine non_hrmt_real_diag_random
...
subroutine give_degeneracies(array,n,shift,n_list,list_degen)
 implicit none
 BEGIN_DOC
 ! subroutines that returns the list of degeneracies in a sorted array 
 !
 END_DOC
 ...
 integer, intent(out) :: list_degen(2,n)
 integer :: i,j,n_degen
 i=1
 n_list = 0
 do while(i.lt.n)
  if(dabs(array(i)-array(i+1)).lt.shift)then
   n_list += 1
   list_degen(1,n_list) = i
   j=1
   do while(dabs(array(i)-array(i+j)).lt.shift.and.i+j.le.n)
    j+=1
   enddo
   n_degen = j
   list_degen(2,n_list) = n_degen
   i+=n_degen
  else 
   i+=1
  endif
