...
subroutine non_hrmt_real_diag(n, A, leigvec, reigvec, n_real_eigv, eigval)
  ...
  if( (accu_nd .gt. threshold) .or. (dabs(accu_d-dble(n_real_eigv)) .gt. threshold) ) then
    ...
    call get_inv_half_svd(S, n_real_eigv, S_inv_half_tmp)

    print*, ' S inv half'
    do i = 1, n_real_eigv
     write(*,'(1000(F16.10,X))') S_inv_half_tmp(i,:)
    enddo

!  double precision, allocatable :: Stmp(:,:), Stmp2(:,:)
!  allocate( Stmp(n_real_eigv,n_real_eigv), Stmp2(n_real_eigv,n_real_eigv) )
 
!  Stmp = 0.d0
!  call dgemm( 'N', 'N', n_real_eigv, n_real_eigv, n_real_eigv, 1.d0          &
!            , S_inv_half_tmp, size(S_inv_half_tmp, 1), S, size(S, 1) &
!            , 0.d0, Stmp, size(Stmp, 1) )
!  Stmp2 = 0.d0
!  call dgemm( 'N', 'N', n_real_eigv, n_real_eigv, n_real_eigv, 1.d0          &
!            , Stmp, size(Stmp, 1), S_inv_half_tmp, size(S_inv_half_tmp, 1) &
...
!   stop
!  endif
!  deallocate(S, Stmp, Stmp2, S_inv_half_tmp)
...
subroutine non_hrmt_bieig(n, A, leigvec, reigvec, n_real_eigv, eigval)
  ...
  if( accu_nd .lt. 1d-8 ) then
    ...
    print *, ' '
    print *, ' right eigenvect aft bi-orhtog' 
    do i = 1, n
      write(*, '(1000(F16.10,X))') reigvec(:,i)
    enddo
    print *, ' left eigenvect aft bi-orthog'
    do i = 1, n
      write(*, '(1000(F16.10,X))') leigvec(:,i)
    enddo
    print *, ' '
