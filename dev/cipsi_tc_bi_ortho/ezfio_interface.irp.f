...
! If true, saves the wave function after the selection, before the diagonalization
  ...
  if (mpi_master) then
    ...
    call ezfio_has_cipsi_tc_bi_ortho_save_wf_after_selection(has)
    if (has) then
      write(6,'(A)') '.. >>>>> [ IO READ: save_wf_after_selection ] <<<<< ..'
      call ezfio_get_cipsi_tc_bi_ortho_save_wf_after_selection(save_wf_after_selection)
    else
      print *, 'cipsi_tc_bi_ortho/save_wf_after_selection not found in EZFIO file'
      stop 1
    endif
  endif
  IRP_IF MPI_DEBUG
...
! Maximum number of allowed open shells. Using -1 selects all determinants
  ...
  if (mpi_master) then
    ...
    call ezfio_has_cipsi_tc_bi_ortho_seniority_max(has)
    if (has) then
      write(6,'(A)') '.. >>>>> [ IO READ: seniority_max ] <<<<< ..'
      call ezfio_get_cipsi_tc_bi_ortho_seniority_max(seniority_max)
    else
      print *, 'cipsi_tc_bi_ortho/seniority_max not found in EZFIO file'
      stop 1
    endif
  endif
  IRP_IF MPI_DEBUG
...
! 1: Hartree-Fock determinant, 2:All determinants of the dominant configuration
  ...
  if (mpi_master) then
    ...
    call ezfio_has_cipsi_tc_bi_ortho_excitation_ref(has)
    if (has) then
      write(6,'(A)') '.. >>>>> [ IO READ: excitation_ref ] <<<<< ..'
      call ezfio_get_cipsi_tc_bi_ortho_excitation_ref(excitation_ref)
    else
      print *, 'cipsi_tc_bi_ortho/excitation_ref not found in EZFIO file'
      stop 1
    endif
  endif
  IRP_IF MPI_DEBUG
...
! Maximum number of excitation with respect to the Hartree-Fock determinant. Using -1 selects all determinants
  ...
  if (mpi_master) then
    ...
    call ezfio_has_cipsi_tc_bi_ortho_excitation_max(has)
    if (has) then
      write(6,'(A)') '.. >>>>> [ IO READ: excitation_max ] <<<<< ..'
      call ezfio_get_cipsi_tc_bi_ortho_excitation_max(excitation_max)
    else
      print *, 'cipsi_tc_bi_ortho/excitation_max not found in EZFIO file'
      stop 1
    endif
  endif
  IRP_IF MPI_DEBUG
...
! Maximum number of excitation for alpha determinants with respect to the Hartree-Fock determinant. Using -1 selects all determinants
  ...
  if (mpi_master) then
    ...
    call ezfio_has_cipsi_tc_bi_ortho_excitation_alpha_max(has)
    if (has) then
      write(6,'(A)') '.. >>>>> [ IO READ: excitation_alpha_max ] <<<<< ..'
      call ezfio_get_cipsi_tc_bi_ortho_excitation_alpha_max(excitation_alpha_max)
    else
      print *, 'cipsi_tc_bi_ortho/excitation_alpha_max not found in EZFIO file'
      stop 1
    endif
  endif
  IRP_IF MPI_DEBUG
...
! Maximum number of excitation for beta determinants with respect to the Hartree-Fock determinant. Using -1 selects all determinants
  ...
  if (mpi_master) then
    ...
    call ezfio_has_cipsi_tc_bi_ortho_excitation_beta_max(has)
    if (has) then
      write(6,'(A)') '.. >>>>> [ IO READ: excitation_beta_max ] <<<<< ..'
      call ezfio_get_cipsi_tc_bi_ortho_excitation_beta_max(excitation_beta_max)
    else
      print *, 'cipsi_tc_bi_ortho/excitation_beta_max not found in EZFIO file'
      stop 1
    endif
  endif
  IRP_IF MPI_DEBUG
