...
! TODO : Put the documentation of the program here
  ...
  call print_exp_small_mu_ground_state 
  call print_exp_pade_mu_0_ground_state
 !call print_E_H_sr_ex_ground_state
end

 subroutine print_exp_small_mu_ground_state   
 implicit none
 ...
 double precision:: mu_loc,sr_hatree_energy_small_mu_contrib_loc
 print*,trim(ezfio_filename)
 character*(128) :: filename
 write (filename, "(I1)")order_derivative_ontop

 output=trim(ezfio_filename)//'.small_mu_exp_order_'//trim(filename)
 output=trim(output)
 ...
 do istate = 1, n_states
  do n_mu = 0,1000
   ...
   sr_hatree_energy_small_mu_contrib_loc = 0.d0

   do n = 0, order_derivative_ontop
    sr_hatree_energy_small_mu_contrib_loc +=  (mu_loc**(2.d0*dble(n)+1.d0)) * sr_hatree_energy_small_mu_coef(n,istate) 
   enddo

   sr_hatree_energy_small_mu_contrib_loc = regular_range_Hartree(istate) + sr_hatree_energy_small_mu_contrib_loc 

   write(i_unit_output,'(10(F16.10,X))')mu_loc,sr_hatree_energy_small_mu_contrib_loc
  enddo
 enddo  
 ...
 subroutine print_exp_pade_mu_0_ground_state   
 implicit none
 BEGIN_DOC
 ! blablablabla
 END_DOC
...
!m=order_derivative_ontop-n
 ...
 provide ezfio_filename 
 integer :: n_mu
 double precision:: mu_loc,e_h_pade_diag_coef_loc
 print*,trim(ezfio_filename)
 character*(128) :: filename
 write (filename, "(I1)")order_derivative_ontop

 output=trim(ezfio_filename)//'.small_mu_pade_order_'//trim(filename)
 output=trim(output)
 print*,'output = ',trim(output)
 i_unit_output = getUnitAndOpen(output,'w')
 ...
 vect_b_temp=0.d0
  ...
  do n_mu = 0,1000
   ...
   nume = 0.d0
   do i= 0,n
    nume += vect_a(i)* mu_loc**(dble(i))
   enddo
   
   denom = 0.d0
   do i= 0,m
    denom += vect_b(i)* mu_loc**(dble(i))
   enddo
  
   e_h_pade_diag_coef_loc = regular_range_Hartree(1) + nume/denom
