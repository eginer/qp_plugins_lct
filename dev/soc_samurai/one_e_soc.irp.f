...
BEGIN_PROVIDER [ double precision, ao_one_e_soc_per_atom, (3,nucl_num,ao_num, ao_num)]
 implicit none
 BEGIN_DOC
 ! ao_one_e_soc_per_atom(coord,i_nucl,j,i) = \alpha^2/2\int dr Z_nucl /|r - R_nucl| [grad_r AO_i(r)] X [grad_r AO_j(r)]_coord
 END_DOC
 integer :: i,j,ipoint,istate,k,l,coord,i_nucl
 double precision :: r_n(3),r_e(3),dis,v_nucl,z_n
 ao_one_e_soc_per_atom = 0.d0
  ...
  do i= 1, ao_num
   do j = 1, ao_num
    do ipoint = 1, n_points_final_grid
     ...
     do coord = 1,3
      ao_one_e_soc_per_atom(coord,i_nucl,j, i) += vec_prod_grad_ao_at_r(coord,ipoint,j,i) * v_nucl * final_weight_at_r_vector(ipoint)
     enddo
    enddo
   enddo
  enddo
...
BEGIN_PROVIDER [ double precision, ao_one_e_soc, (ao_num, ao_num,3)]
 implicit none
 BEGIN_DOC
 ! ao_one_e_soc_per_atom(j,i,coord) = \alpha^2/2 \sum_{nucl} \int dr Z_nucl /|r - R_nucl| [grad_r AO_i(r)] X [grad_r AO_j(r)]_coord
 END_DOC
 implicit none 
 integer :: i,j,ipoint,istate,k,l,coord,i_nucl
 ao_one_e_soc_per_atom = 0.d0
 do coord = 1, 3
  do i = 1, ao_num
   do j = 1, ao_num
    do i_nucl = 1, nucl_num
     ao_one_e_soc_per_atom(j,i,coord) += ao_one_e_soc_per_atom(coord,i_nucl, j, i)
    enddo
   enddo
  enddo
 enddo
...
BEGIN_PROVIDER [ double precision, mo_one_e_soc, (mo_num, mo_num,3)]
 implicit none
 BEGIN_DOC
 ! mo_one_e_soc_per_atom(j,i,coord) = \alpha^2/2 \sum_{nucl} \int dr Z_nucl /|r - R_nucl| [grad_r MO_i(r)] X [grad_r MO_j(r)]_coord
 END_DOC
 integer :: i
 mo_one_e_soc = 0.d0
 do i = 1, 3
  call ao_to_mo(ao_one_e_soc(1,1,i),ao_num,mo_one_e_soc(1,1,i),mo_num)
 enddo
END_PROVIDER
