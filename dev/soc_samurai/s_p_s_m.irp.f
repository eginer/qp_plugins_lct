...
! open_shell = 0_bit_kind
 ...
 ! set to 1 the bit corresponding to orb_j into the alpha string
 call set_bit_to_integer(orb_j,det_out(1,1),N_int)
 ! clear the bit corresponding to orb_j into the beta string
...
subroutine s_minus_det_orb_j(det_in,orb_j,det_out,phase)
 implicit none
 BEGIN_DOC
...
! open_shell = 0_bit_kind
 ...
 enddo
 ! check if the excitation is possible 
 ! ==> is the orbital orb_j occupied in open_shell alpha
 det_orb_j = 0_bit_kind
 call set_bit_to_integer(orb_j,det_orb_j,N_int)
 j = 0
 do i = 1, N_int
  j+= popcnt(iand(det_orb_j(i),open_a_b(i,1)))
 enddo
 if(j.ne.1)then
  return
  phase = 0.d0
 ...
 call set_bit_to_integer(orb_j,det_out(1,2),N_int)
 ! clear the bit corresponding to orb_j into the alpha string
 call clear_bit_to_integer(orb_j,det_out(1,1),N_int)
...
! call debug_det(open_a_b,N_int)
 ...
 do i = 1, n_occ_ab(1)
  j = occ(i,1)
  call s_minus_det_orb_j(det_in,j,det_tmp,phase_tmp)
  if(phase_tmp .ne. 0.d0)then
   ndet_out += 1
   det_out(:,:,ndet_out) = det_tmp(:,:) 
   phase(ndet_out) = phase_tmp
