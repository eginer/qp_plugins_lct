...
! big_mat(i,k,j,l) += \int dr1 \phi_i(r1) \phi_k(r1) \int dr2 erf(\mu(r1) r12)/r12 \phi_j(r2) \phi_l(r2)
 ...
 allocate(a_mat(n_points_extra_final_grid,ao_num,ao_num))

 do i = 1, ao_num
  do k = 1, ao_num
   do ipoint = 1, n_points_extra_final_grid
    weight = final_weight_at_r_vector_extra(ipoint)
    a_mat(ipoint,k,i) = aos_in_r_extra_grid_array_transp(ipoint,k) * aos_in_r_extra_grid_array_transp(ipoint,i) * weight
   enddo
  enddo
 enddo

 ! erf(mu(r) * r12)/r12
 call dgemm("N","N",ao_num*ao_num,ao_num*ao_num,n_points_extra_final_grid,1.d0,erf_mu_r12_inv_r12_rk_extra(1,1,1),ao_num*ao_num & 
                   ,a_mat(1,1,1),n_points_extra_final_grid,1.d0,big_mat,ao_num*ao_num)
...
! big_mat(i,k,j,l) += \int dr1 \phi_i(r1) \phi_k(r1) \int dr2 erf(\mu(r1) r12)/r12 \phi_j(r2) \phi_l(r2)
 ...
 call dgemm("N","N",ao_num*ao_num,ao_num*ao_num,n_points_extra_final_grid,1.d0,erfc_extra_grid_rk(1,1,1),ao_num*ao_num & 
                   ,a_mat(1,1,1),n_points_extra_final_grid,1.d0,big_mat,ao_num*ao_num)
