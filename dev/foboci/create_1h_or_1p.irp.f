subroutine create_restart_and_1h(i_hole)
 implicit none 
 BEGIN_DOC
! you enter with an orbital number "i_hole" which must be in the inactive space
!
! then you create all single excitations on top of psi_det  that does 
!
! i_hole --> active space 
!
! and psi_det, N_det are touched  
 END_DOC
 use bitmasks
  ...
  do j = 1, n_act_orb
   ...
   do i = 1, N_det
    do ispin = 1,2
     ...
     enddo
     call do_single_excitation(key_tmp,i_hole,i_part_act,ispin,i_ok)
     if(i_ok .ne. 1)cycle
     n_new_det +=1
    enddo
   enddo
...
subroutine create_restart_and_1p(i_particle)
 implicit none 
 BEGIN_DOC
! you enter with an orbital number "i_particle" which must be in the virtual space
!
! then you create all single excitations on top of psi_det  that does 
!
! active space --> i_particle
!
! and psi_det, N_det are touched  
 END_DOC
   ...
   do i = 1, N_det
    do ispin = 1,2
     do k = 1, N_int
      ...
      key_tmp(k,2) = psi_det(k,2,i)
     enddo
     call do_single_excitation(key_tmp,i_hole_act,i_particle,ispin,i_ok)
     if(i_ok .ne. 1)cycle
     n_new_det +=1
    enddo
   enddo
