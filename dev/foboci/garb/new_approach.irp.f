...
!   call all_single_split_for_1p(dressing_matrix_1h1p,dressing_matrix_1h2p)
 ...
 logical :: orb_taken_good_det(mo_num)
 ...
 do i = 1, n_inact_orb
  ...
  do j = 1, n_act_orb
   ...
   ispin = 2
   key_tmp = ref_bitmask
   call do_single_excitation(key_tmp,iorb,jorb,ispin,i_ok)
   if(i_ok .ne.1)cycle
 ...
 do i = 1, n_virt_orb
  ...
  do j = 1, n_act_orb
   ...
   ispin = 1
   key_tmp = ref_bitmask
   call do_single_excitation(key_tmp,jorb,iorb,ispin,i_ok)
   if(i_ok .ne.1)cycle
   is_already_present = .False.
   H_array = 0.d0
   call i_h_j(key_tmp,key_tmp,N_int,hij)
...
!print*,'norm = ',norm
 ...
 do i = 1, n_core_orb
  i_core = list_core(i)
  one_e_dm_mo(i_core,i_core) = 10.d0
  do j = i+1, n_core_orb
   j_core = list_core(j)
   one_e_dm_mo(i_core,j_core) = 0.d0
   one_e_dm_mo(j_core,i_core) = 0.d0
  enddo
  do j = 1, n_inact_orb
   iorb = list_inact(j)
   one_e_dm_mo(i_core,iorb) = 0.d0
   one_e_dm_mo(iorb,i_core) = 0.d0
  enddo
  do j = 1, n_act_orb
   iorb = list_act(j)
   one_e_dm_mo(i_core,iorb) = 0.d0
 ...
 do i = 1, n_inact_orb
  ...
  do j = i+1, n_inact_orb 
   ...
   one_e_dm_mo(i_inact,j_inact) = 0.d0
   one_e_dm_mo(j_inact,i_inact) = 0.d0
  enddo
 enddo
 ...
 do i = 1, n_inact_orb
  ...
  do j = 1, n_virt_orb 
   j_virt = list_virt(j)
   one_e_dm_mo(i_inact,j_virt) = 0.d0
   one_e_dm_mo(j_virt,i_inact) = 0.d0
  enddo
 enddo
 ...
 do i = 1, n_virt_orb
  ...
  do j = i+1, n_virt_orb 
   j_virt = list_virt(j)
   one_e_dm_mo(i_virt,j_virt) = 0.d0
   one_e_dm_mo(j_virt,i_virt) = 0.d0
  enddo
 enddo
 ...
 do i = 1,n_act_orb
  ...
  do j = 1, n_inact_orb
   jorb = list_inact(j)
   if(dabs(one_e_dm_mo(iorb,jorb)).gt.threshold_lmct)then
    print*,'INACTIVE  '
    print*,'DM ',iorb,jorb,dabs(one_e_dm_mo(iorb,jorb))
   endif
  enddo
  do j = 1, n_virt_orb
   jorb = list_virt(j)
   if(dabs(one_e_dm_mo(iorb,jorb)).gt.threshold_mlct)then
    print*,'VIRT      '
    print*,'DM ',iorb,jorb,dabs(one_e_dm_mo(iorb,jorb))
   endif
  enddo
 ...
 do i = 1, mo_num
  do j = i+1, mo_num
   if(dabs(one_e_dm_mo(i,j)).le.threshold_fobo_dm)then
      one_e_dm_mo(i,j) = 0.d0
      one_e_dm_mo(j,i) = 0.d0
   endif
  enddo
 enddo
 ...
 call mo_as_eigvectors_of_mo_matrix(one_e_dm_mo,size(one_e_dm_mo,1),size(one_e_dm_mo,2),label,sign,.true.)
 soft_touch mo_coef
 call save_mos
