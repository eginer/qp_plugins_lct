...
subroutine update_density_matrix_osoci
 implicit none
 BEGIN_DOC
 ! one_e_dm_mo_alpha_osoci += Delta rho alpha
 ! one_e_dm_mo_beta_osoci  += Delta rho beta
 END_DOC
 integer :: i,j
 integer :: iorb,jorb
 do i = 1, mo_num
  do j = 1, mo_num
   one_e_dm_mo_alpha_osoci(i,j) = one_e_dm_mo_alpha_osoci(i,j) + (one_e_dm_mo_alpha_average(i,j) - one_e_dm_mo_alpha_generators_restart(i,j))
   one_e_dm_mo_beta_osoci(i,j) = one_e_dm_mo_beta_osoci(i,j) + (one_e_dm_mo_beta_average(i,j) - one_e_dm_mo_beta_generators_restart(i,j))
  enddo
 enddo
...
subroutine update_density_matrix_beta_osoci_read(array)
 ...
 integer :: i,j
 integer :: iorb,jorb
 double precision :: array(mo_num)
 do i = 1, mo_num
   j = list_act(1)
   one_e_dm_mo_beta_osoci(i,j) += array(i)
   one_e_dm_mo_beta_osoci(j,i) += array(i)
   one_e_dm_mo_beta_osoci(i,i) += array(i) * array(i)
 enddo
...
subroutine update_density_matrix_alpha_osoci_read(array)
 ...
 do i = 1, mo_num
   j = list_act(1)
   one_e_dm_mo_alpha_osoci(i,j) += array(i)
   one_e_dm_mo_alpha_osoci(j,i) += array(i)
   one_e_dm_mo_alpha_osoci(i,i) += array(i) * array(i)
 enddo
...
subroutine initialize_density_matrix_osoci
 implicit none
 one_e_dm_mo_alpha_osoci = one_e_dm_mo_alpha_generators_restart
 one_e_dm_mo_beta_osoci  = one_e_dm_mo_beta_generators_restart
end
...
subroutine rescale_density_matrix_osoci(norm)
 ...
 do i = 1, mo_num
  do j = 1,mo_num
   one_e_dm_mo_alpha_osoci(i,j) = one_e_dm_mo_alpha_osoci(i,j) * norm_tmp
   one_e_dm_mo_beta_osoci(j,i) = one_e_dm_mo_beta_osoci(j,i) * norm_tmp
  enddo
 enddo
end
...
subroutine save_osoci_natural_mos
 ...
 allocate(tmp_bis(size(one_e_dm_mo_alpha_osoci,1),size(one_e_dm_mo_alpha_osoci,2)))
 allocate (occ(N_int*bit_kind_size,2))

 ! Negation to have the occupied MOs first after the diagonalization
 tmp_bis = -one_e_dm_mo_alpha_osoci - one_e_dm_mo_beta_osoci
...
subroutine check_symetry(i_hole,thr,test)
 ...
 accu = 0.d0
 do i = 1, n_act_orb
  accu += dabs(mo_one_e_integrals(i_hole,list_act(i)))
 enddo
 if(accu.gt.thr)then
  test = .True.
 else
...
subroutine check_symetry_1h1p(i_hole,i_part,thr,test)
 ...
 integer :: i,j,k,l
 double precision :: accu
 accu = dabs(mo_one_e_integrals(i_hole,i_part))
 if(accu.gt.thr)then
  test = .True.
 else
  test = .false.
