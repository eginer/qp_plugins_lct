 ...
 do j = 1, mo_num ! 2 
  do i = 1, mo_num ! 
   ...
   do k = 1, mo_num ! 2 
    ...
    do l = 1, mo_num ! 1
     !                               2 2 1 1 
     two_dm += full_occ_2_rdm_ab_mo(l,k,i,j,istate)  * mos_array_r1(i) * mos_array_r1(j) * mos_array_r1(k) * mos_array_r1(l)
     two_dm_laplacian += full_occ_2_rdm_ab_mo(l,k,j,i,istate) * mos_array_r1(i) * mos_array_r1(l) * (lapl_k * mos_array_r1(j) + 2.d0 * scalar_grad + lapl_j * mos_array_r1(k))
    enddo
   enddo
  enddo
 enddo
 two_dm_laplacian = 1.d0/3.d0 * two_dm_laplacian
 
 total_dm = two_dm + 0.5d0 * two_dm_laplacian * r12*r12 
 
end


subroutine spherical_averaged_two_dm_HF_at_second_order(r1,r12,two_dm,two_dm_laplacian,total_dm)
 implicit none
 double precision, intent(in) :: r1(3),r12
 double precision, intent(out) :: two_dm,two_dm_laplacian,total_dm
 double precision, allocatable :: mos_array_r1(:)
