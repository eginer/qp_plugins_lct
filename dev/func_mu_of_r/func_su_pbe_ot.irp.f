...
! print*,'needs_eff_two_e_ints_su_pbe_ot = ',needs_eff_two_e_ints_su_pbe_ot
 ...
 do istate = 1, N_states
  do ipoint = 1, n_points_final_grid
   ...
   e_c_md_basis_su_pbe_ot(istate) += ec_srmuPBE * weight
  enddo
 enddo
...
!3
 BEGIN_PROVIDER[double precision, aos_vc_alpha_sr_pbe_w_n2  , (ao_num,n_points_final_grid,N_states)]
&BEGIN_PROVIDER[double precision, aos_vc_beta_sr_pbe_w_n2   , (ao_num,n_points_final_grid,N_states)]
&BEGIN_PROVIDER[double precision, aos_d_vc_alpha_sr_pbe_w_n2  , (ao_num,n_points_final_grid,N_states)]
&BEGIN_PROVIDER[double precision, aos_d_vc_beta_sr_pbe_w_n2   ,  (ao_num,n_points_final_grid,N_states)]
...
! intermediates to compute the sr_pbe potentials 
 ...
 aos_d_vc_alpha_sr_pbe_w_n2 = 0.d0
 aos_d_vc_beta_sr_pbe_w_n2 = 0.d0
 do istate = 1, N_states
  do ipoint = 1, n_points_final_grid
   r(1) = final_grid_points(1,ipoint)
   r(2) = final_grid_points(2,ipoint)
   ...
   do j = 1, ao_num
    aos_vc_alpha_sr_pbe_w_n2(j,ipoint,istate) = decdrho_a * aos_in_r_array(j,ipoint)
    aos_vc_beta_sr_pbe_w_n2 (j,ipoint,istate) = decdrho_b * aos_in_r_array(j,ipoint)
   enddo
   do j = 1, ao_num
    do m = 1,3
     aos_d_vc_alpha_sr_pbe_w_n2(j,ipoint,istate) += contrib_grad_ca(m) * aos_grad_in_r_array_transp(m,j,ipoint)
     aos_d_vc_beta_sr_pbe_w_n2 (j,ipoint,istate) += contrib_grad_cb(m) * aos_grad_in_r_array_transp(m,j,ipoint)
    enddo
   enddo
  enddo
 enddo
 ...
 integer                        :: istate
   ...
   do istate = 1, N_states
     ...
     call dgemm('N','T',ao_num,ao_num,n_points_final_grid,1.d0,                   &
                 aos_vc_alpha_sr_pbe_w_n2(1,1,istate),size(aos_vc_alpha_sr_pbe_w_n2,1), &
                 aos_in_r_array,size(aos_in_r_array,1),1.d0,                      &
                 pot_scal_alpha_ao_su_pbe_ot(1,1,istate),size(pot_scal_alpha_ao_su_pbe_ot,1))
     ! correlation beta
     call dgemm('N','T',ao_num,ao_num,n_points_final_grid,1.d0,                   &
                 aos_vc_beta_sr_pbe_w_n2(1,1,istate),size(aos_vc_beta_sr_pbe_w_n2,1),   &
                 aos_in_r_array,size(aos_in_r_array,1),1.d0,                      &
...
&BEGIN_PROVIDER [double precision, pot_grad_beta_ao_su_pbe_ot,(ao_num,ao_num,N_states)]
   ...
   do istate = 1, N_states
       ...
       call dgemm('N','N',ao_num,ao_num,n_points_final_grid,1.d0,                     &
                  aos_d_vc_alpha_sr_pbe_w_n2(1,1,istate),size(aos_d_vc_alpha_sr_pbe_w_n2,1),  &
       ...
       ! correlation beta
       call dgemm('N','N',ao_num,ao_num,n_points_final_grid,1.d0,                     &
                  aos_d_vc_beta_sr_pbe_w_n2(1,1,istate),size(aos_d_vc_beta_sr_pbe_w_n2,1),    &
                  aos_in_r_array_transp,size(aos_in_r_array_transp,1),1.d0,           &
