...
! spin_population(i,j) = rho_AO(alpha)(i,j) - rho_AO(beta)(i,j) * <AO_i|AO_j>
 ...
 do i = 1, ao_num
  do j = 1, ao_num
   spin_population(j,i) = one_e_spin_density_ao(i,j) * ao_overlap(i,j)
  enddo
 enddo
END_PROVIDER
...
! spin_population(i,j) = rho_AO(alpha)(i,j) - rho_AO(beta)(i,j) * <AO_i|AO_j>
 ...
 do i = 1, ao_num
  do j = 1, ao_num
   electronic_population_alpha(j,i) = one_e_dm_mo_alpha_average(i,j) * ao_overlap(i,j)
   electronic_population_beta(j,i) = one_e_dm_mo_beta_average(i,j) * ao_overlap(i,j)
  enddo
 enddo
...
subroutine print_mulliken_sd
 ...
 do i = 1, ao_num
  ...
  write(*,'(1X,I3,1X,A4,1X,I2,1X,A4,1X,F10.7)')i,trim(element_name(int(nucl_charge(ao_nucl(i))))),ao_nucl(i),trim(l_to_character(ao_l(i))),spin_gross_orbital_product(i)
 enddo
 print*,'sum = ',accu
 accu = 0.d0
 ...
 do i = 0,  ao_l_max
  ...
  print*,' ',trim(l_to_character(i)),spin_population_angular_momentum(i)
 print*,'sum = ',accu
 enddo
 ...
 do j = 1,nucl_num
  ...
  do i = 0,  ao_l_max
   ...
   write(*,'(1X,I3,1X,A4,1X,A4,1X,F10.7)')j,trim(element_name(int(nucl_charge(j)))),trim(l_to_character(i)),spin_population_angular_momentum_per_atom(i,j)
   print*,'sum = ',accu
  enddo
 enddo
