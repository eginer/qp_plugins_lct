BEGIN_PROVIDER [double precision, spin_density_at_nucleus, (nucl_num)]
 implicit none
 BEGIN_DOC
! value of the spin density at each nucleus 
 ...
 do i = 1, nucl_num 
  ...
  do j = 1, ao_num
   do k = 1, ao_num
    accu += one_e_spin_density_ao(k,j) * aos_array(k) * aos_array(j)
   enddo
  enddo
  spin_density_at_nucleus(i) = accu
 enddo
END_PROVIDER

 BEGIN_PROVIDER [double precision, spin_density_at_nucleus_from_mo, (nucl_num)]
&BEGIN_PROVIDER [double precision, spin_density_at_nucleus_contrib_per_mo, (nucl_num,mo_num)]
 implicit none
 BEGIN_DOC
! value of the spin density at each nucleus 
 END_DOC
 ...
 do i = 1, nucl_num 
  ...
  double precision :: mo_values(mo_num)
  accu = 0.d0
  r(1:3) = nucl_coord(i,1:3)
  call give_all_aos_at_r(r,aos_array)
  spin_density_at_nucleus_from_mo(i) = 0.d0
  do k = 1, mo_num
   mo_values(k) = 0.d0
   do j = 1, ao_num
  ...
  enddo
  do k = 1, mo_num
   spin_density_at_nucleus_contrib_per_mo(i,k) = 0.d0
   do m = 1, mo_num
    contrib = one_e_spin_density_mo(k,m) * mo_values(k) * mo_values(m)
    spin_density_at_nucleus_from_mo(i) += contrib
    spin_density_at_nucleus_contrib_per_mo(i,k) += contrib
   enddo
  enddo
 enddo
END_PROVIDER

 BEGIN_PROVIDER [double precision, spin_density_at_nucleus_contrib_mo, (nucl_num,mo_num,mo_num)]
&BEGIN_PROVIDER [double precision, spin_density_at_nucleus_contrib_mo_test, (nucl_num)]
 implicit none
 BEGIN_DOC
! value of the spin density at each nucleus 
 END_DOC
 integer :: i,j,k,l,m
 spin_density_at_nucleus_contrib_mo_test = 0.d0
 do i = 1, nucl_num 
  double precision :: r(3),accu,aos_array(ao_num)
    ...
    accu = 0.d0
       do j = 1, ao_num
        ...
        do l = 1, ao_num
         ...
         accu += one_e_spin_density_mo(k,m) * aos_array(l) * aos_array(j) * c_j1
        enddo
       enddo
    spin_density_at_nucleus_contrib_mo(i,k,m) = accu
    spin_density_at_nucleus_contrib_mo_test(i) += accu
   enddo 
  enddo
 enddo
END_PROVIDER
...
! isotropic hyperfine coupling constants among the various atoms
 ...
 integer :: i
 do i = 1, nucl_num
   iso_hcc_mhz(i)   = conversion_factor_mhz_hcc(nint(nucl_charge(i))) * spin_density_at_nucleus(i)  !* 0.5d0
   iso_hcc_gauss(i) = conversion_factor_gauss_hcc(nint(nucl_charge(i))) * spin_density_at_nucleus(i)!* 0.5d0
   iso_hcc_cm_1(i)  = conversion_factor_cm_1_hcc(nint(nucl_charge(i))) * spin_density_at_nucleus(i) !*0.5d0
 enddo
