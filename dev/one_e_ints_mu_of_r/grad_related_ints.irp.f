...
BEGIN_PROVIDER [ double precision, v_ij_erf_rk_transp, (n_points_final_grid, ao_num, ao_num)]
 implicit none
 BEGIN_DOC
! int dr phi_i(r) phi_j(r) (erf(mu(R) |r - R| - 1)/|r - R|
 END_DOC
 ...
 !$OMP DEFAULT (NONE)            &
 !$OMP PRIVATE (i,j,ipoint,mu,r,int_mu,int_coulomb) & 
 !$OMP SHARED (ao_num,n_points_final_grid,mu_of_r_for_ints,v_ij_erf_rk_transp,final_grid_points)
 !$OMP DO SCHEDULE (dynamic)
 do i = 1, ao_num
  do j = i, ao_num
   do ipoint = 1, n_points_final_grid
     ...
     int_mu = NAI_pol_mult_erf_ao(i,j,mu,r)
     int_coulomb = NAI_pol_mult_erf_ao(i,j,1.d+9,r)
     v_ij_erf_rk_transp(ipoint,j,i)= (int_mu - int_coulomb )
   enddo
  enddo
 enddo
 !$OMP END DO
 ...
 do i = 1, ao_num
  do j = 1, i-1
   do ipoint = 1, n_points_final_grid
     v_ij_erf_rk_transp(ipoint,j,i)= v_ij_erf_rk_transp(ipoint,i,j)
    enddo
   enddo
  enddo

 call wall_time(wall1)
 print*,'wall time for v_ij_erf_rk_transp  ',wall1 - wall0
END_PROVIDER 
...
BEGIN_PROVIDER [ double precision, x_v_ij_erf_rk_transp_bis, (n_points_final_grid,ao_num, ao_num,3)]
 implicit none
 BEGIN_DOC
! int dr x * phi_i(r) phi_j(r) (erf(mu(R) |r - R|) - 1)/|r - R|
 END_DOC
 ...
 do m = 1, 3
  do i = 1, ao_num
   do j = 1, ao_num
    do ipoint = 1, n_points_final_grid
     x_v_ij_erf_rk_transp_bis(ipoint,j,i,m)= x_v_ij_erf_rk_tmp(m,j,i,ipoint)
    enddo
   enddo
  enddo
 enddo
...
BEGIN_PROVIDER [ double precision, d_dx_v_ij_erf_rk_tmp, (3,n_points_final_grid,ao_num, ao_num)]
 implicit none
 BEGIN_DOC
! d_dx_v_ij_erf_rk_tmp(m,R,j,i) = int dr phi_j(r)) (erf(mu(R) |r - R|) - 1)/|r - R| d/dx (phi_i(r) 
!
! with m == 1 -> d/dx , m == 2 -> d/dy , m == 3 -> d/dz
 ...
 !$OMP DEFAULT (NONE)            &
 !$OMP PRIVATE (i,j,ipoint,mu,r,ints,m,ints_coulomb) & 
 !$OMP SHARED (ao_num,n_points_final_grid,mu_of_r_for_ints,d_dx_v_ij_erf_rk_tmp,final_grid_points)
 !$OMP DO SCHEDULE (dynamic)
 do i = 1, ao_num
  do j = 1, ao_num
   do ipoint = 1, n_points_final_grid
    ...
    do m = 1, 3
     d_dx_v_ij_erf_rk_tmp(m,ipoint,j,i) = ( ints(m) - ints_coulomb(m))
    enddo
   enddo
  enddo
 enddo
 ...
 call wall_time(wall1)
 print*,'wall time for d_dx_v_ij_erf_rk_tmp',wall1 - wall0


END_PROVIDER 

BEGIN_PROVIDER [ double precision, d_dx_v_ij_erf_rk, (n_points_final_grid,ao_num, ao_num,3)]
 implicit none
 BEGIN_DOC
! d_dx_v_ij_erf_rk_tmp(j,i,R,m) = int dr phi_j(r)) (erf(mu(R) |r - R|) - 1)/|r - R| d/dx (phi_i(r) 
!
! with m == 1 -> d/dx , m == 2 -> d/dy , m == 3 -> d/dz
 END_DOC
 integer :: i,j,ipoint,m
 ...
 do i = 1, ao_num
  do j = 1, ao_num
   do m = 1, 3
    do ipoint = 1, n_points_final_grid
     d_dx_v_ij_erf_rk(ipoint,j,i,m)= d_dx_v_ij_erf_rk_tmp(m,ipoint,j,i)
    enddo
   enddo
  enddo
 enddo
 ...
 call wall_time(wall1)
 print*,'wall time for d_dx_v_ij_erf_rk',wall1 - wall0

END_PROVIDER 

BEGIN_PROVIDER [ double precision, x_d_dx_v_ij_erf_rk_tmp, (3,n_points_final_grid,ao_num, ao_num)]
 implicit none
 BEGIN_DOC
! x_d_dx_v_ij_erf_rk_tmp(m,j,i,R) = int dr x phi_j(r)) (erf(mu(R) |r - R|) - 1)/|r - R| d/dx (phi_i(r) 
!
! with m == 1 -> d/dx , m == 2 -> d/dy , m == 3 -> d/dz
 END_DOC
 integer :: i,j,ipoint,m
 ...
 !$OMP DEFAULT (NONE)            &
 !$OMP PRIVATE (i,j,ipoint,mu,r,ints,m,ints_coulomb) & 
 !$OMP SHARED (ao_num,n_points_final_grid,mu_of_r_for_ints,x_d_dx_v_ij_erf_rk_tmp,final_grid_points)
 !$OMP DO SCHEDULE (dynamic)
 do i = 1, ao_num
  do j = 1, ao_num
   do ipoint = 1, n_points_final_grid
    ...
    do m = 1, 3
     x_d_dx_v_ij_erf_rk_tmp(m,ipoint,j,i) = ( ints(m) - ints_coulomb(m))
    enddo
   enddo
  enddo
 enddo
 ...
 call wall_time(wall1)
 print*,'wall time for x_d_dx_v_ij_erf_rk_tmp',wall1 - wall0


END_PROVIDER 

BEGIN_PROVIDER [ double precision, x_d_dx_v_ij_erf_rk, (n_points_final_grid,ao_num, ao_num,3)]
 implicit none
 BEGIN_DOC
! x_d_dx_v_ij_erf_rk_tmp(j,i,R,m) = int dr x phi_j(r)) (erf(mu(R) |r - R|) - 1)/|r - R| d/dx (phi_i(r) 
!
! with m == 1 -> d/dx , m == 2 -> d/dy , m == 3 -> d/dz
 END_DOC
 integer :: i,j,ipoint,m
 ...
 do i = 1, ao_num
  do j = 1, ao_num
   do ipoint = 1, n_points_final_grid
    do m = 1, 3
     x_d_dx_v_ij_erf_rk(ipoint,j,i,m)= x_d_dx_v_ij_erf_rk_tmp(m,ipoint,j,i)
    enddo
   enddo
  enddo
 enddo
 ...
 call wall_time(wall1)
 print*,'wall time for x_d_dx_v_ij_erf_rk',wall1 - wall0
