subroutine mo_bielec_integrals_ijkl_r3_index(i,j,k,l,i1)
  use map_module
  implicit none
  BEGIN_DOC
...
BEGIN_PROVIDER [ logical, mo_bielec_integrals_ijkl_r3_in_map ]
  use map_module
  implicit none
  ...
  mo_bielec_integrals_ijkl_r3_in_map = .True.
  if (read_mo_integrals_ijkl_r3) then
   !print*,'Reading the MO integrals_ijkl_r3'
   !call map_load_from_disk(trim(ezfio_filename)//'/work/mo_ints_ijkl_r3',mo_integrals_ijkl_r3_map)
   !print*, 'MO integrals_ijkl_r3 provided'
    return
  else
    PROVIDE ao_bielec_integrals_ijkl_r3_in_map
  endif
  
  !  call four_index_transform_block(ao_integrals_ijkl_r3_map,mo_integrals_ijkl_r3_map, &
  !      mo_coef, size(mo_coef,1),                                      &
  !      1, 1, 1, 1, ao_num, ao_num, ao_num, ao_num,                    &
  !      1, 1, 1, 1, mo_num, mo_num, mo_num, mo_num)
   call add_integrals_to_map_ijkl_r3(full_ijkl_bitmask_4)
    integer*8                      :: get_mo_ijkl_r3_map_size, mo_ijkl_r3_map_size
    mo_ijkl_r3_map_size = get_mo_ijkl_r3_map_size()
    
! print*,'Molecular integrals ijkl_r3 provided:'
! print*,' Size of MO ijkl_r3 map           ', map_mb(mo_integrals_ijkl_r3_map) ,'MB'
! print*,' Number of MO ijkl_r3 integrals: ',  mo_ijkl_r3_map_size
 !if (write_mo_integrals_ijkl_r3) then
 !  call ezfio_set_work_empty(.False.)
 !  call map_save_to_disk(trim(ezfio_filename)//'/work/mo_ints_ijkl_r3',mo_integrals_ijkl_r3_map)
 !  call ezfio_set_integrals_ijkl_r3_disk_access_mo_integrals_ijkl_r3("Read")
 !endif
...
&BEGIN_PROVIDER [ double precision, mo_bielec_integral_ijkl_r3_jj_anti_from_ao, (mo_tot_num,mo_tot_num) ]
  ...
  if (.not.do_direct_integrals) then
    PROVIDE ao_bielec_integrals_ijkl_r3_in_map mo_coef
  endif
  
  mo_bielec_integral_ijkl_r3_jj_from_ao = 0.d0
  mo_bielec_integral_ijkl_r3_jj_exchange_from_ao = 0.d0
  ...
  do s=1,ao_num
    do q=1,ao_num
      ...
      if (do_direct_integrals) then
        ...
        do r=1,ao_num
          call compute_ao_bielec_integrals_ijkl_r3(q,r,s,ao_num,int_value)
          do p=1,ao_num
          ...
          enddo
          call compute_ao_bielec_integrals_ijkl_r3(q,s,r,ao_num,int_value)
          do p=1,ao_num
            integral = int_value(p)
            if (abs(integral) > ao_integrals_threshold) then
              !DIR$ VECTOR ALIGNED
        ...
        do r=1,ao_num
          call get_ao_bielec_integrals_ijkl_r3_non_zero(q,r,s,ao_num,int_value,int_idx,n)
          do pp=1,n
            p = int_idx(pp)
          ...
          enddo
          call get_ao_bielec_integrals_ijkl_r3_non_zero(q,s,r,ao_num,int_value,int_idx,n)
          do pp=1,n
            p = int_idx(pp)
            integral = int_value(pp)
            if (abs(integral) > ao_integrals_threshold) then
  ...
  mo_bielec_integral_ijkl_r3_jj_anti_from_ao = mo_bielec_integral_ijkl_r3_jj_from_ao - mo_bielec_integral_ijkl_r3_jj_exchange_from_ao
...
subroutine clear_mo_ijkl_r3_map
  implicit none
  BEGIN_DOC
  ! Frees the memory of the MO map
  END_DOC
  call map_deinit(mo_integrals_ijkl_r3_map)
  FREE mo_integrals_ijkl_r3_map mo_bielec_integral_ijkl_r3_jj mo_bielec_integral_ijkl_r3_jj_anti
  FREE mo_bielec_integral_ijkl_r3_jj_exchange mo_bielec_integrals_ijkl_r3_in_map
  
  
end

subroutine provide_all_mo_integrals_ijkl_r3
  implicit none
  provide mo_integrals_ijkl_r3_map mo_bielec_integral_ijkl_r3_jj mo_bielec_integral_ijkl_r3_jj_anti
  provide mo_bielec_integral_ijkl_r3_jj_exchange mo_bielec_integrals_ijkl_r3_in_map
  
end


subroutine add_integrals_to_map_ijkl_r3(mask_ijkl)
  use bitmasks
  implicit none
    ...
    if (thread_num == 0) then
      if (wall_2 - wall_0 > 1.d0) then
        ...
        print*, 100.*float(l1)/float(ao_num), '% in ',               &
            wall_2-wall_1, 's', map_mb(mo_integrals_ijkl_r3_map) ,'MB'
      endif
    endif
  enddo
  !$OMP END DO NOWAIT
  ...
  !$OMP END PARALLEL
  call map_merge(mo_integrals_ijkl_r3_map)
  
  call wall_time(wall_2)
  call cpu_time(cpu_2)
  integer*8                      :: get_mo_ijkl_r3_map_size, mo_map_size
  mo_map_size = get_mo_ijkl_r3_map_size()
  
  deallocate(list_ijkl)
  
  
  print*,'Molecular ijkl_r3 integrals provided:'
  print*,' Size of MO ijkl_r3 map           ', map_mb(mo_integrals_ijkl_r3_map) ,'MB'
  print*,' Number of MO ijkl_r3 integrals: ',  mo_map_size
