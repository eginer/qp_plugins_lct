subroutine ao_bielec_integrals_ijkl_r3_in_map_slave_tcp(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_bielec_integrals_ijkl_r3_in_map_slave(0,i)
end


subroutine ao_bielec_integrals_ijkl_r3_in_map_slave_inproc(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_bielec_integrals_ijkl_r3_in_map_slave(1,i)
end
...
subroutine ao_bielec_integrals_ijkl_r3_in_map_slave(thread,iproc)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
! Computes a buffer of integrals
  ...
  do 
    ...
    call compute_ao_integrals_ijkl_r3_jl(j,l,n_integrals,buffer_i,buffer_value) 
...
subroutine ao_bielec_integrals_ijkl_r3_in_map_collector(zmq_socket_pull)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
...
IRP_ENDIF
      ...
      call insert_into_ao_integrals_ijkl_r3_map(n_integrals,buffer_i,buffer_value)
      accu += n_integrals
      if (task_id /= 0) then
  ...
  control = get_ao_ijkl_r3_map_size(ao_integrals_ijkl_r3_map)
