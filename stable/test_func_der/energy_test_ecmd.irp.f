subroutine ecmdsrPBEn2_test(mu_in,rho_a,rho_b,grad_rho_a_2,grad_rho_b_2,grad_rho_a_b,rho2,ec_srmuPBE,decdrho_a,decdrho_b, decdrho, decdgrad_rho_a_2,decdgrad_rho_b_2,decdgrad_rho_a_b, decdgrad_rho_2,decdrho2)
  ...
  double precision, intent(in)  :: rho_a(n_points_final_grid),rho_b(n_points_final_grid),grad_rho_a_2(n_points_final_grid),grad_rho_b_2(n_points_final_grid),grad_rho_a_b(n_points_final_grid), rho2(n_points_final_grid)
  double precision, intent(out) :: ec_srmuPBE,decdrho_a(n_points_final_grid),decdrho_b(n_points_final_grid),decdgrad_rho_a_2(n_points_final_grid),decdgrad_rho_b_2(n_points_final_grid),decdgrad_rho_a_b(n_points_final_grid), decdgrad_rho_2(n_points_final_grid), decdrho(n_points_final_grid), decdrho2(n_points_final_grid)!, decdrho2_a, decdrho2_b
  double precision              :: rho(n_points_final_grid), m(n_points_final_grid)
  double precision              :: mu
  ...
  double precision              :: pi, c, thr, weight
  integer                       :: i
  ec_srmuPBE       = 0.d0
  decdrho_a        = 0.d0
  decdrho_b        = 0.d0
  ...
  do i = 1, n_points_final_grid
   ...
   if(rho(i).lt.1.d-10)then
    return
   else if(rho2(i)/(rho(i)**2) .lt. 1.d-6)then
    return
   endif
   m(i) = rho_a(i) - rho_b(i)
...
! correlation PBE standard and on-top pair distribution 
   call rho_ab_to_rho_oc(rho_a(i),rho_b(i),rho_o,rho_c)
   call grad_rho_ab_to_grad_rho_oc(grad_rho_a_2(i),grad_rho_b_2(i),grad_rho_a_b(i),grad_rho_o_2,grad_rho_2,grad_rho_o_c)
...
! calculation of derivatives 
  ...
  if(dabs(rho2(i)).lt.1.d-10)then
   ...
   dbetadrho = decPBEdrho/(c*rho2(i)) ! - (ecPBE/(c*rho2**2))*dn2_UEGdrho
  endif
  ddenomdrho = dbetadrho*mu**3

  decdrho(i) = decPBEdrho/denom - ecPBE*ddenomdrho/(denom**2)
  decdrho_a(i) = decdrho(i)
  decdrho_b(i) = decdrho(i)
...
!dec/((dgradn)^2)
  ...
  if(dabs(rho2(i)).lt.1.d-10)then
   ...
   dbetadgrad_rho_2 = decPBEdgrad_rho_2/(c*rho2(i))
  endif
  ddenomdgrad_rho_2 = dbetadgrad_rho_2*mu**3

  decdgrad_rho_2(i)   = decPBEdgrad_rho_2/denom - ecPBE*ddenomdgrad_rho_2/(denom**2)
  decdgrad_rho_a_2(i) = decdgrad_rho_2(i) ! + decdgrad_n_m + decdgrad_m_2
  decdgrad_rho_b_2(i) = decdgrad_rho_2(i) ! - decdgrad_n_m + decdgrad_m_2
  decdgrad_rho_a_b(i) = 2.d0*decdgrad_rho_2(i) ! - 2.d0*decdgrad_m_2 

  !dec/dn2

  if(dabs(rho2(i)).gt.1.d-10)then
   ddenomdrho2 = - (mu**3)* ecPBE/(c*rho2(i)**2)
  else
   ddenomdrho2 = - (mu**3)* ecPBE * 1.d-20
  endif

  decdrho2(i) = - ecPBE*ddenomdrho2/(denom**2)
  ! decdrho2_a = decdrho2
  ! decdrho2_b = decdrho2
  enddo
  end 
