...
!   call energy_xc_pbe_test (rho_a, rho_b, grad_rho_a, grad_rho_b, k, l, ex_pbe_test, ec_pbe_test, vx_pbe_test, vc_pbe_test)
   int_vx_pbe_test += delta_gamma_i_j(k,l)*(potential_x_alpha_mo_pbe(k,l) + potential_x_beta_mo_pbe(k,l))
   int_vc_pbe_test += delta_gamma_i_j(k,l)*(potential_c_alpha_mo_pbe(k,l) + potential_c_beta_mo_pbe(k,l))
  enddo
 enddo
end
