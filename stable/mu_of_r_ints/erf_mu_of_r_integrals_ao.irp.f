...
! int dr1 dr2 AO_i(r1) AO_j(r1) erf(mu(r1) |r1-r2|) / |r1-r2| AO_k(r2) AO_l(r2)
 ...
 do i_point = 1, n_points_final_grid
  ...
  erf_mu_of_r_ao += 0.5d0 * aos_array(i) * aos_array(j) * NAI_pol_mult_erf_ao(k,l,mu_of_r_for_ints_vector(i_point),r) * final_weight_at_r_vector(i_point)
  erf_mu_of_r_ao += 0.5d0 * aos_array(k) * aos_array(l) * NAI_pol_mult_erf_ao(j,i,mu_of_r_for_ints_vector(i_point),r) * final_weight_at_r_vector(i_point)
 enddo
 ...
 do i_point = 1, n_points_final_grid
  ...
  r(2) = final_grid_points(2,i_point) 
  r(3) = final_grid_points(3,i_point) 
  tmp = NAI_pol_mult_erf_ao(k,l,mu_of_r_for_ints_vector(i_point),r) 
...
!call dgemm('N','N',ao_num,ao_num,n_points_final_grid,1.d0,v_array,size(v_array,1),aos_in_r_array_transp,size(aos_in_r_array_transp,1),1.d0,integrals,size(integrals,1))
 do i_point = 1, n_points_final_grid
  do i = 1, ao_num
   do j = 1, ao_num
    integrals(j,i) += v_vector(i_point) * aos_in_r_array(i,i_point) * aos_in_r_array(j,i_point) * final_weight_at_r_vector(i_point)
   enddo
  enddo
 enddo
 end
