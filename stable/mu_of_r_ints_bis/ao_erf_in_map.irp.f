subroutine ao_int_erf_mu_of_r_integrals_in_map_slave_tcp(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_int_erf_mu_of_r_integrals_in_map_slave(0,i)
end


subroutine ao_int_erf_mu_of_r_in_map_slave_inproc(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_int_erf_mu_of_r_integrals_in_map_slave(1,i)
end



subroutine ao_int_erf_mu_of_r_integrals_in_map_slave(thread,iproc)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
! Computes a buffer of integrals
  ...
  do
    ...
    call compute_ao_erf_mu_of_r_integrals_jl(j,l,n_integrals,buffer_i,buffer_value)
...
subroutine ao_int_erf_mu_of_r_integrals_in_map_collector(zmq_socket_pull)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
...
IRP_ENDIF
      ...
      call insert_into_ao_int_erf_mu_of_r_map(n_integrals,buffer_i,buffer_value)
      accu += n_integrals
      if (task_id /= 0) then
  ...
  control = get_ao_erf_mu_of_r_map_size(ao_int_erf_mu_of_r_map)
