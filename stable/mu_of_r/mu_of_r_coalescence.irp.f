...
&BEGIN_PROVIDER [double precision, mu_of_r_psi_coal, (n_points_integration_angular,n_points_radial_grid,nucl_num) ]
 ...
 BEGIN_PROVIDER [double precision, mu_of_r_hf_valencecoal_vector, (n_points_final_grid) ]
&BEGIN_PROVIDER [double precision, mu_of_r_hf_valencecoal, (n_points_integration_angular,n_points_radial_grid,nucl_num) ]
 implicit none 
 BEGIN_DOC
 ! mu_of_r and mu_average computation 
 END_DOC
 ...
 double precision :: r(3)
 double precision :: cpu0,cpu1,local_potential,two_bod
 print*,'providing the mu_of_r_hf_valencecoal_vector ...'
 call wall_time(cpu0)
 r = 0.d0
 call f_HF_ab(r,r,local_potential,two_bod)
 !$OMP PARALLEL DO &
 !$OMP DEFAULT (NONE)  &
 !$OMP PRIVATE (i_point,r,local_potential,two_bod) & 
 !$OMP ShARED (n_points_final_grid,final_grid_points,mu_of_r_hf_valencecoal_vector) 
 do i_point = 1, n_points_final_grid
  r(1) = final_grid_points(1,i_point)
  r(2) = final_grid_points(2,i_point)
  r(3) = final_grid_points(3,i_point)
  ...
  mu_of_r_hf_valencecoal_vector(i_point) =  local_potential * dsqrt(dacos(-1.d0)) * 0.5d0
 enddo
 !$OMP END PARALLEL DO
 do i_point = 1, n_points_final_grid
  k = index_final_points(1,i_point)
  i = index_final_points(2,i_point)
  j = index_final_points(3,i_point)
  mu_of_r_hf_valencecoal(k,i,j) = mu_of_r_hf_valencecoal_vector(i_point)
 enddo
 call wall_time(cpu1)
 print*,'Time to provide mu_of_r_hf_valencecoal_vector = ',cpu1-cpu0
 END_PROVIDER 
