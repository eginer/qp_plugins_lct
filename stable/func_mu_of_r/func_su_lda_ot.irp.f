 BEGIN_PROVIDER [logical, needs_eff_two_e_ints_su_lda_ot]
 implicit none
 BEGIN_DOC
 ! needs_eff_two_e_ints_su_lda_ot = True because the self consistent procedure for this functional requires to write effective two electron integrals
 END_DOC
 needs_eff_two_e_ints_su_lda_ot = .True.

 END_PROVIDER

!BEGIN_PROVIDER[double precision, e_c_md_basis_su_lda_ot, (N_states) ]
!implicit none
!BEGIN_DOC
!
...
!e_c_md_basis_su_lda_ot = ecmd_pbe_on_top_su_mu_of_r
!END_PROVIDER


 BEGIN_PROVIDER [double precision, pot_basis_alpha_mo_su_lda_ot,(mo_num,mo_num,N_states)]
&BEGIN_PROVIDER [double precision, pot_basis_beta_mo_su_lda_ot,(mo_num,mo_num,N_states)]
   implicit none
 BEGIN_DOC
 ...
 ! 
 END_DOC 
 call ao_to_mo(pot_basis_alpha_ao_su_lda_ot,ao_num,pot_basis_alpha_mo_su_lda_ot,mo_num)
 call ao_to_mo(pot_basis_beta_ao_su_lda_ot,ao_num,pot_basis_beta_mo_su_lda_ot,mo_num)
END_PROVIDER 

!1
 BEGIN_PROVIDER [double precision, pot_basis_alpha_ao_su_lda_ot,(ao_num,ao_num,N_states)]
&BEGIN_PROVIDER [double precision, pot_basis_beta_ao_su_lda_ot,(ao_num,ao_num,N_states)]
   implicit none
 BEGIN_DOC
 ...
 END_DOC 
   ...
   do istate = 1, n_states 
    do i = 1, ao_num
     do j = 1, ao_num
      pot_basis_alpha_ao_su_lda_ot(j,i,istate) = pot_scal_alpha_ao_su_lda_ot(j,i,istate) !+ pot_grad_alpha_ao_su_lda_ot(j,i,istate) + pot_grad_alpha_ao_su_lda_ot(i,j,istate)
      pot_basis_beta_ao_su_lda_ot(j,i,istate) = pot_scal_beta_ao_su_lda_ot(j,i,istate) !+ pot_grad_beta_ao_su_lda_ot(j,i,istate) + pot_grad_beta_ao_su_lda_ot(i,j,istate)
     enddo
    enddo
   enddo
...
!3
 BEGIN_PROVIDER[double precision, e_c_md_basis_su_lda_ot, (N_states) ]
&BEGIN_PROVIDER[double precision, aos_vc_alpha_su_lda_ot_w  , (ao_num,n_points_final_grid,N_states)]
&BEGIN_PROVIDER[double precision, aos_vc_beta_su_lda_ot_w   , (ao_num,n_points_final_grid,N_states)]
!&BEGIN_PROVIDER[double precision, aos_d_vc_alpha_su_lda_ot_w  , (ao_num,n_points_final_grid,N_states)]
!&BEGIN_PROVIDER[double precision, aos_d_vc_beta_su_lda_ot_w   ,  (ao_num,n_points_final_grid,N_states)]
&BEGIN_PROVIDER[double precision, d_dn2_e_cmd_su_lda_ot, (n_points_final_grid,N_states) ]
 implicit none
 include 'constants.include.F'
 BEGIN_DOC
! Intermediates to compute the su-pbe-ot potentials 
!
! An important quantity is the "d_dn2_e_cmd_su_lda_ot" which is the functional derivative with respect to the on-top 
!
...
! NOTICE THAT SUCH QUANTITY IS RETURNED WITH NORMALIZATION N(N-1) WHICH IS SUITED FOR DIRECT MODIFICATION OF V_ijkl
 ...
 double precision :: ec_srmuLDAn,mu
 ...
 e_c_md_basis_su_lda_ot = 0.d0
 !aos_d_vc_alpha_su_lda_ot_w = 0.d0
 !aos_d_vc_beta_su_lda_ot_w = 0.d0
 do istate = 1, N_states
...
!   enddo
   ...
   do j = 1, ao_num
    ...
    aos_vc_beta_su_lda_ot_w (j,ipoint,istate) = decdrho_b * aos_in_r_array(j,ipoint)
   enddo
 !  do j = 1, ao_num
 !   do m = 1,3
 !    aos_d_vc_alpha_su_lda_ot_w(j,ipoint,istate) += contrib_grad_ca(m) * aos_grad_in_r_array_transp(m,j,ipoint)
 !    aos_d_vc_beta_su_lda_ot_w (j,ipoint,istate) += contrib_grad_cb(m) * aos_grad_in_r_array_transp(m,j,ipoint)
 !   enddo
 !  enddo
  enddo
 enddo
...
!4
 BEGIN_PROVIDER [double precision, pot_scal_alpha_ao_su_lda_ot, (ao_num,ao_num,N_states)]
&BEGIN_PROVIDER [double precision, pot_scal_beta_ao_su_lda_ot, (ao_num,ao_num,N_states)]
 implicit none
 integer                        :: istate
   BEGIN_DOC
   ...
   !
   END_DOC
   pot_scal_alpha_ao_su_lda_ot = 0.d0
   pot_scal_beta_ao_su_lda_ot = 0.d0
   double precision               :: wall_1,wall_2
   call wall_time(wall_1)
   do istate = 1, N_states
     ! correlation alpha
     call dgemm('N','T',ao_num,ao_num,n_points_final_grid,1.d0,                   &
                 aos_vc_alpha_su_lda_ot_w(1,1,istate),size(aos_vc_alpha_su_lda_ot_w,1), &
                 aos_in_r_array,size(aos_in_r_array,1),1.d0,                      &
                 pot_scal_alpha_ao_su_lda_ot(1,1,istate),size(pot_scal_alpha_ao_su_lda_ot,1))
     ! correlation beta
     call dgemm('N','T',ao_num,ao_num,n_points_final_grid,1.d0,                   &
                 aos_vc_beta_su_lda_ot_w(1,1,istate),size(aos_vc_beta_su_lda_ot_w,1),   &
                 aos_in_r_array,size(aos_in_r_array,1),1.d0,                      &
                 pot_scal_beta_ao_su_lda_ot(1,1,istate),size(pot_scal_beta_ao_su_lda_ot,1))
...
!5
!BEGIN_PROVIDER [double precision, pot_grad_alpha_ao_su_lda_ot,(ao_num,ao_num,N_states)]
!&BEGIN_PROVIDER [double precision, pot_grad_beta_ao_su_lda_ot,(ao_num,ao_num,N_states)]
!  implicit none
!  BEGIN_DOC
...
!  double precision               :: wall_1,wall_2
!  call wall_time(wall_1)
!  pot_grad_alpha_ao_su_lda_ot = 0.d0
!  pot_grad_beta_ao_su_lda_ot = 0.d0
!  do istate = 1, N_states
!      ! correlation alpha
!      call dgemm('N','N',ao_num,ao_num,n_points_final_grid,1.d0,                     &
!                 aos_d_vc_alpha_su_lda_ot_w(1,1,istate),size(aos_d_vc_alpha_su_lda_ot_w,1),  &
!                 aos_in_r_array_transp,size(aos_in_r_array_transp,1),1.d0,           &
!                 pot_grad_alpha_ao_su_lda_ot(1,1,istate),size(pot_grad_alpha_ao_su_lda_ot,1))
!      ! correlation beta
!      call dgemm('N','N',ao_num,ao_num,n_points_final_grid,1.d0,                     &
!                 aos_d_vc_beta_su_lda_ot_w(1,1,istate),size(aos_d_vc_beta_su_lda_ot_w,1),    &
!                 aos_in_r_array_transp,size(aos_in_r_array_transp,1),1.d0,           &
!                 pot_grad_beta_ao_su_lda_ot(1,1,istate),size(pot_grad_beta_ao_su_lda_ot,1))
!  enddo
!  
!call wall_time(wall_2)
