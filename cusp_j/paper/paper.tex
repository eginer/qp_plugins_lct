...
The first part of our study concerns the convergence of the ground state eigenvalue of $\tilde{H}[\mu]$ with respect to the basis set, and more precisely the dependence on $\mu$ of such convergence. 
...
\end{equation}
and its right eigenvectors and eigenvalues satisfy of course
\begin{equation}
 \tilde{H}[\mu]^{\basis} \ket{\phimub} = E_i^{\basis}[\mu] \ket{\phimub}. 
\end{equation}

As pointed in section \ref{sec:h_mu_lim}, by varying $\mu$ one can go from a relatively usual repulsive Hamiltonian (even though it is non hermitian an non divergent), passing by an almost attractive Hamiltonian and even to a purely attractive Hamiltonian. 
...
and so will $\ket{\phimub}$ and $E_i^{\basis}[\mu]$. 
Therefore, we would like to study the mathematical behaviour of $\ket{\phimub}$ and $E_i^{\basis}[\mu]$ as a function of both $\basis$ and $\mu$ to see whether the use of the new form of Jastrow factor $j(r_{12},\mu)$ can improve the convergence toward the exact ground state energy and eigenfunction. 
\subsection{$\mu$ dependence of the convergence of the total energy with respect to the basis set }
\subsubsection{Computational details}
We implemented all integrals needed for the computation of the matrix elements of $\tilde{H}[\mu]$ on a usual orbital basis set (see annexes for more details) in the case of an atomic two electron systems, together with the modification of the Slater rules due to the presence of the non hermitian term in $\tilde{H}[\mu]$. 
All implementation was done as a plugin of the program quantum package\cite{QP2}. 
Once the matrix elements of $\tilde{H}[\mu]$ are built on a given MO basis, we solve the two-body problem giving full flexibility to $\Phi_\mu$ (\textit{i.e.} it is expressed as a linear combination of all possible Slater determinants within a given Hilbert space defined with a MO basis) and use an non hermitian eigensolver to obtain both right and left eigenvectors. If not explicitly mentioned, the canonical RHF MOs are used in this work. 
...
when enlarging the basis set. Also, qualitatively, one can observe that there are two regimes of $\mu$: that ranging from $\mu=0.05$ and $\mu=0.3$ where the $E_0^\mu$ is always below $E_0$ and those larger than $\mu=0.35$ where $E_0^\mu$ converges from above, as if the variational principle still applied. 
Second, as $\mu$ increases, the difference between $E_0^\mu$ and $E_0^\text{FCI}$ in a given basis set diminishes, 
which is expected as in the limit $\mu \rightarrow \infty$, one obtains that  $\tilde{H}[\mu] \rightarrow H$. 
...
To quantify better such observations, one can look at the basis set from which the error with respect to the exact non relativistic energy is smaller, in absolute value, than 1mH. 
For $0.3\ge\mu\ge0.4$ one can see that the error with respect to $E_0$ is never larger, in absolute value, than 1mH whatever the basis set used. For $0.5\ge \mu \ge 1.0$ such accuracy is reached at the aug-cc-pVTZ level. For $1.6\ge \mu \ge 3.0$, such accuracy is reached at the aug-cc-pVQZ level, whenever such accuracy is reached at the aug-cc-pV5Z level using the regular FCI Hamiltonian. 
Therefore one can conclude that the use of the Jastrow factor $j(r_{12};\mu)$ in the transcorrelated framework can strongly improve the speed of convergence of the total energy of the helium atom, specially for $\mu \in [0.4, 1.0]$ where an error below 1mH is reached within the aug-cc-pVTZ basis set, whereas such an error is reached at the aug-cc-pV5Z basis set. 
\begin{figure*}
 \label{fig_conv_e_mu}
...
\subsubsection{Numerical results: ground state eigenvectors}
...
\begin{equation}
 \psiex(\br{}_1,\br{}_2) = e^{j(r_{12},\mu)} \phimu(\br{}_1,\br{}_2). 
\end{equation}
Therefore, having obtained in a given basis set $\basis$ the right eigenvector $\phimub$, one can estimate the exact ground state eigenvector by 
\begin{equation}
 \psimub(\br{}_1,\br{}_2) = \frac{1}{\sqrt{\mathcal{N}_{\basis}^{\mu}}}e^{j(r_{12},\mu)}\phimub(\br{}_1,\br{}_2)
\end{equation}
where the normalization factor $\mathcal{N}_{\basis}^{\mu}$ is imply
\begin{equation}
